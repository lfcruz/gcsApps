        ...
        $validQueue = pgQResult("select * from t_queue where queue_id = $1", array(2));
        var_dump($validQueue);
                if(!empty($validQueue)){
                    while(true){
                        ...
                        if(!empty($targetsDS)){
                            foreach ($targetsDS as $qRecord){
                                $result = sentMessage($qRecord['target'], $qRecord['telcoid'], $qRecord['message'], $validQueue[0]['campaing_type_id']);
                                if($result){
                        ...
                        sleep(120);
                        $validQueue = pgQResult("select * from t_queue where queue_id = $1", array($_GET['pool']));
                    }
