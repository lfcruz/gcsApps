...
{
    global $mwHeader;
    $url = "http://$enviroment:6088/cardholders";
...
{
    global $mwHeader;
    $url = "http://$enviroment:6088/cardholder/$docType/$docNumber/phones/$phoneNumber";
...
function vcashEnrollment($enviroment,$cardholderStructure)
{
    ...
    if ($result == "0000"){
        $activationResult = vCashActivation($enviroment, $cardholderStructure["idType"], $cardholderStructure["id"], $cardholderStructure["telephone"]);
        if (array_key_exists('error',$activationResult)) {
            $result = '9903';
        }        
        else {
            $result = 'A'.$activationResult['activation-code'];
        }
    }
    return $result;
}
...
{
    global $mwHeader;
    $url = "http://$enviroment:6088/cardholder/$docType/$docNumber/financial";
    $requestResult = do_post_request($url, json_encode($financialStructure), $mwHeader, 'POST');
    $jsonResult = json_decode($requestResult,true);
    return $jsonResult;    
}

function vCashActivation($enviroment,$docType,$docNumber,$phoneNumber) 
{
    global $mwHeader;
    $url = "http://$enviroment:6088/cardholder/$docType/$docNumber/activation/$phoneNumber";
    $requestResult = do_post_request($url,null,$mwHeader,'POST');
    $jsonResult = json_decode($requestResult,true);
    return $jsonResult;
}
