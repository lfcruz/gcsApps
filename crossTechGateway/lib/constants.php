...
define("DTO_USER_SCOPES","select distinct sr.api_tag as scopes from api_security.users_roles ur inner join api_security.roles rl on (rl.id = ur.role_id) inner join api_security.roles_definition rd on (rd.roles_id = rl.id) inner join api_security.functions fu on (fu.id = rd.functions_id) inner join api_security.services sr on (sr.id = fu.service_id) where ur.user_id = $1 and sr.active is true and fu.active is true");
define("DTO_USER_AUDIENCE","select distinct fu.api_tag as audiences from api_security.users_roles ur inner join api_security.roles rl on (rl.id = ur.role_id) inner join api_security.roles_definition rd on (rd.roles_id = rl.id) inner join api_security.functions fu on (fu.id = rd.functions_id) inner join api_security.services sr on (sr.id = fu.service_id) where ur.user_id = $1 and sr.active is true and fu.active is true");
define("DTO_USER_ROLES","select distinct rl.api_tag as roles from api_security.users_roles ur inner join api_security.roles rl on (rl.id = ur.role_id) inner join api_security.roles_definition rd on (rd.roles_id = rl.id) inner join api_security.functions fu on (fu.id = rd.functions_id) inner join api_security.services sr on (sr.id = fu.service_id) where ur.user_id = $1 and sr.active is true and fu.active is true");
